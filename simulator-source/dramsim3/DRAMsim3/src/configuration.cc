...
Address Config::AddressMapping(uint64_t hex_addr) const {
    ...
    // Pouya: address mapping for bg is xored with middle bit of column address
    // TODO: comment lines below if you want to use normal address mapping
    // XOR first bit of bg (bit 0) with second bit of co (bit 1)
    int bg_lsb = bg & 0x1;
    int col_bit1 = (co >> 1) & 0x1; // 2nd bit of col (address[1])
    int new_bg_lsb = bg_lsb ^ col_bit1;
    bg = (bg & ~0x1) | new_bg_lsb;

    // XOR second bit of bg (bit 1) with first bit of ro (bit 0)
    int bg_bit1 = (bg >> 1) & 0x1;
    int ro_bit0 = ro & 0x1;
    int new_bg_bit1 = bg_bit1 ^ ro_bit0;
    bg = (bg & ~(0x1 << 1)) | (new_bg_bit1 << 1);
    

    // XOr fist bit of ba (bit 0) with second bit of ro (bit 1)
    int ba_lsb = ba & 0x1;
    int ro_bit1 = (ro >> 1) & 0x1;
    int new_ba_lsb = ba_lsb ^ ro_bit1;
    ba = (ba & ~0x1) | new_ba_lsb;

    // XOR second bit of ba (bit 1) with third bit of ro (bit 2)
    int ba_bit1 = (ba >> 1) & 0x1;
    int ro_bit2 = (ro >> 2) & 0x1;
    int new_ba_bit1 = ba_bit1 ^ ro_bit2;
    ba = (ba & ~(0x1 << 1)) | (new_ba_bit1 << 1);


    // XOR first bit of rank (bit 0) with fourth bit of ro (bit 3)
    int rank_lsb = rank & 0x1;
    int ro_bit3 = (ro >> 3) & 0x1;
    int new_rank_lsb = rank_lsb ^ ro_bit3;
    rank = (rank & ~0x1) | new_rank_lsb;

    // the address mapping is based on the skylake mapping in the study below:
