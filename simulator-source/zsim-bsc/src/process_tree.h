...
class ProcessTreeNode : public GlobAlloc {
    private:
        ...
        const bool terminateAllOnExit;
        const bool fastForwardPeersOnRoiEnd;
        volatile bool pendingForceFastForward;
        volatile bool forceUnsyncedFastForward;
    ...
    public:
        ProcessTreeNode(uint32_t _procIdx, uint32_t _groupIdx, bool _inFastForward, bool _inPause, const SyncedFastForwardMode& _syncedFastForward,
                        ...
                        bool _terminateAllOnExit, bool _fastForwardPeersOnRoiEnd, const g_vector<bool>& _mask, const g_vector<uint64_t>& _ffiPoints, const g_string& _syscallBlacklistRegex, const char*_patchRoot)
            : patchRoot(_patchRoot), procIdx(_procIdx), groupIdx(_groupIdx), curChildren(0), heartbeats(0), started(false), inFastForward(_inFastForward),
              inPause(_inPause), terminateAllOnExit(_terminateAllOnExit), fastForwardPeersOnRoiEnd(_fastForwardPeersOnRoiEnd), pendingForceFastForward(false), forceUnsyncedFastForward(false), restartsLeft(_restarts), syncedFastForward(_syncedFastForward), clockDomain(_clockDomain), portDomain(_portDomain), dumpHeartbeats(_dumpHeartbeats), dumpsResetHeartbeats(_dumpsResetHeartbeats), mask(_mask), ffiPoints(_ffiPoints), syscallBlacklistRegex(_syscallBlacklistRegex) {}
        ...
        ProcessTreeNode* getNextChild() {
            if (curChildren == children.size()) { //allocate a new child
                ...
                child->curChildren = 0;
                child->heartbeats = 0;
                child->pendingForceFastForward = false;
                child->forceUnsyncedFastForward = false;
        ...
        bool shouldTerminateAllOnExit() const { return terminateAllOnExit; }
        bool shouldForcePeersFastForwardOnRoiEnd() const { return fastForwardPeersOnRoiEnd; }
        void requestFastForward(bool unsynced = false) {
            if (unsynced) forceUnsyncedFastForward = true;
            (void)__sync_lock_test_and_set(&pendingForceFastForward, true);
        }
        void forceFastForwardUnsynced() { forceUnsyncedFastForward = true; }
        bool consumePendingFastForwardRequest() { return __sync_bool_compare_and_swap(&pendingForceFastForward, true, false); }
        ...
        inline bool getSyncedFastForward() const {
            if (forceUnsyncedFastForward) return false;
