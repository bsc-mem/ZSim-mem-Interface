...
void OOOCoreRecorder::recordAccess(uint64_t curCycle, uint64_t dispatchCycle, uint64_t respCycle) {
    ...
    processAccessRecord(tr, curCycle, dispatchCycle, respCycle);
}

void OOOCoreRecorder::processAccessRecord(const TimingRecord& tr, uint64_t curCycle, uint64_t dispatchCycle, uint64_t respCycle) {
    if (IsGet(tr.type)) {
        assert(tr.endEvent);
        addIssueEvent(curCycle);

        //Delay between issue and dispatch
        DelayEvent* dDisp = new (eventRecorder) DelayEvent(dispatchCycle - curCycle);
        dDisp->setMinStartCycle(curCycle);

        //Dispatch event
        OOODispatchEvent* dispEv = new (eventRecorder) OOODispatchEvent(0, dispatchCycle);
        ...
        DelayEvent* dUp = new (eventRecorder) DelayEvent(tr.reqCycle - dispatchCycle);
...
bool OOOCoreRecorder::recordPrefetch(const TimingRecord& tr) {
    if (!lastEvProduced || state != RUNNING) {
        return false;
    }

    uint64_t zllCycle = tr.reqCycle - gapCycles;
    uint64_t lastZll = lastEvProduced->zllStartCycle;
    if (unlikely(zllCycle + gapCycles < lastEvProduced->getMinStartCycle())) {
        return false;
    }

    uint32_t issueDelay = (zllCycle > lastZll) ? (zllCycle - lastZll) : 0;
    DelayEvent* dIssue = new (eventRecorder) DelayEvent(issueDelay);
    dIssue->setMinStartCycle(lastEvProduced->getMinStartCycle());
    TimingEvent* anchor = lastEvProduced->addChild(dIssue, eventRecorder);
    anchor->addChild(tr.startEvent, eventRecorder);
