...
bool ProcessTreeNode::notifyEnd() {
    ...
    if (restartsLeft && !zinfo->terminationConditionMet) {
        ...
        if (terminateAllOnExit) {
            bool triggered = !zinfo->terminationConditionMet && __sync_bool_compare_and_swap(&zinfo->terminationConditionMet, false, true);
            if (triggered) {
                info("terminateAll triggered by procIdx %d (group %d)", procIdx, groupIdx);
                if (zinfo->sched) {
...
static void PopulateLevel(Config& config, const std::string& prefix, std::vector<ProcessTreeNode*>& globProcVector, ProcessTreeNode* parent, uint32_t& procIdx, uint32_t& groupIdx) {
    ...
    while (true) {
        ...
        bool terminateAllOnExit = config.get<bool>(p_ss.str() + ".terminateAll", false);
        bool fastForwardPeersOnRoiEnd = config.get<bool>(p_ss.str() + ".fastForwardPeersOnRoiEnd", false);
        ...
        ProcessTreeNode* ptn = new ProcessTreeNode(procIdx, groupIdx, startFastForwarded, startPaused, syncedFastForward, clockDomain, portDomain, dumpHeartbeats, dumpsResetHeartbeats, restarts, terminateAllOnExit, fastForwardPeersOnRoiEnd, mask, ffiPoints, syscallBlacklistRegex, gpr);
...
void CreateProcessTree(Config& config) {
    ProcessTreeNode* rootNode = new ProcessTreeNode(-1, -1, false, false, SFF_NEVER, 0, 0, 0, false, 0, false, false, g_vector<bool> {},  g_vector<uint64_t> {}, g_string {}, nullptr);
    uint32_t procIdx = 0;
    uint32_t groupIdx = 0;
