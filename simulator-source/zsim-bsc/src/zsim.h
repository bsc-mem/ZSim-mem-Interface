...
#if defined(_WITH_RAMULATOR_) && defined(_WITH_RAMULATOR2_)
#error "Both _WITH_RAMULATOR_ and _WITH_RAMULATOR2_ are defined. Only one may be enabled at a time."
#endif
...
struct GlobSimInfo {
    ...
    // Addresses and file paths for key DSOs (zsim, dramsim3, ramulator2, ...)
    struct DebugLibsInfo debugLibs;
    ...
    g_vector<std::function<void()>>* simEndHandlers;
};
