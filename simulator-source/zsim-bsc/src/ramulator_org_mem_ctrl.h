...
#ifndef RAMULATOR_ORG_MEM_CTRL_H_
#define RAMULATOR_ORG_MEM_CTRL_H_
...
class RamulatorOrgAccEvent;
class RamulatorOrg : public MemObject {
  private:
    ...
    std::multimap<uint64_t, RamulatorOrgAccEvent*> inflightRequests;
  ...
  public:
    RamulatorOrg(std::string config_file, unsigned num_cpus, unsigned cache_line_size, uint32_t _minLatency, uint32_t _domain, const g_string& _name, bool pim_mode,  const string& application, unsigned _cpuFreq, bool _record_memory_trace, bool networkOverhead, bool _useClockDivider, const std::vector<uint32_t>& trackedCores);
    ~RamulatorOrg();
    void finish();
    ...
    void enqueue(RamulatorOrgAccEvent* ev, uint64_t cycle);
  ...
  private:
    ...
    vector<RamulatorOrgAccEvent> ramulatorAccEvent;
    set<uint64_t> inflightCheck;
    set<uint64_t> inflightCheck2;
    map<uint64_t, uint64_t> addr_counter;

    std::list<RamulatorOrgAccEvent*> overflowQueue;
  ...
  // DAMOV-extended: core tracking APIs
    ...
    void handleTrackedRequestIssue(RamulatorOrgAccEvent* ev);
    void handleTrackedRequestComplete(RamulatorOrgAccEvent* ev, uint64_t lat, bool isWrite);
    void publishTrackedCoreStats();
    void configureTrackedCores(const std::vector<uint32_t>& trackedCores);
};

#endif  // _WITH_RAMULATOR_
#endif  // RAMULATOR_ORG_MEM_CTRL_H_
