 ...
 *
 * FIXME: For now, mostly hardcoded entry sizes; default is 128 lines (8KB with 64-byte lines).
 * TODO: Adapt to use weave models
 */
class StreamPrefetcher : public BaseCache {
    public:
        static constexpr uint32_t kLinesPerEntry = 64;
        static constexpr uint32_t kPageShift = 6;
        static constexpr uint32_t kPageMask = kLinesPerEntry - 1;
    private:
        struct Entry {
            ...
            AccessTimes times[kLinesPerEntry];
            std::bitset<kLinesPerEntry> valid;
        ...
        Counter profPrefetchRecorderAttached, profPrefetchRecorderBypassed;
        Counter profPrefetchLeadCycles, profPrefetchLeadSamples;
        Counter profPageRunLengthTotal, profPageRunCount;

        g_vector<MemObject*> parents;
        BaseCache* child;
        uint32_t childId;
        g_string name;
        Address lastAccessPage;
        bool hasLastAccessPage;
        uint64_t currentPageRunLength;

        void invalidateLine(Address lineAddr);
        MemObject* selectParent(Address lineAddr) const;

    public:
        explicit StreamPrefetcher(const g_string& _name)
            : timestamp(0), name(_name), lastAccessPage(0), hasLastAccessPage(false), currentPageRunLength(0) {}
