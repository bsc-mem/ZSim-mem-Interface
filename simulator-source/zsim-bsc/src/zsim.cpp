...
void RequestPeersFastForward(ProcessTreeNode* origin) {
    for (uint32_t i = 0; i < zinfo->numProcs; i++) {
        ProcessTreeNode* node = zinfo->procArray[i];
        if (!node || node == origin) continue;
        node->requestFastForward(true /*unsynced*/);
    }
}
...
uint32_t TakeBarrier(uint32_t tid, uint32_t cid) {
    ...
    if (!procTreeNode->isInFastForward() && procTreeNode->consumePendingFastForwardRequest()) {
        info("Process %d forced into fast-forward", procTreeNode->getProcIdx());
        EnterFastForward();
    }
...
VOID SimEnd() {
    ...
    if (procIdx == 0) {
        ...
        if (zinfo->sched) { 
            ...
            for (const auto& handler: *(zinfo->simEndHandlers)) {
                handler();
            }
        }
    }
    ...
    switch (op) {
        ...
        case ZSIM_MAGIC_OP_ROI_END:
            if (!zinfo->ignoreHooks) {
                //TODO: Test whether this is thread-safe
                futex_lock(&zinfo->ffLock);
                bool forcePeers = procTreeNode->shouldForcePeersFastForwardOnRoiEnd();
                bool wasSynced = procTreeNode->getSyncedFastForward();
                bool allowFastForward = !wasSynced || forcePeers;
                if (!procTreeNode->isInFastForward()) {
                    if (allowFastForward) {
                        if (forcePeers) {
                            procTreeNode->forceFastForwardUnsynced();
                        }
                ...
                futex_unlock(&zinfo->ffLock);
                if (forcePeers) {
                    RequestPeersFastForward(procTreeNode);
                }
            }
            return;
