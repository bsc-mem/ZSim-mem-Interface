...
uint32_t MESIBottomCC::getNetworkLatency(MemReq& req) {
    if (!network) return 0;

    if (isLLC) {
        auto targetName = dynamic_cast<SplitAddrMemory*>(parents[0])->getTargetFor(req.lineAddr)->getName();
        return network->getRTT(name.c_str(), targetName, false);
    } else {
        auto id = getParentId(req.lineAddr);
        return network->getRTT(name.c_str(), parents[id]->getName(), false);
    }
}

void MESIBottomCC::init(const g_vector<MemObject*>& _parents, Network* _network, const char* _name) {
    name = _name;
    network = _network;

    // TODO: check if the copy of g_vector class can be used instead of the for loop
    parents.resize(_parents.size());
    for (uint32_t p = 0; p < parents.size(); p++) {
        parents[p] = _parents[p];
    }

    // check if the parent is a mem type (mem splitter)
    bool singleParent = parents.size() == 1;
    if (singleParent && dynamic_cast<SplitAddrMemory*>(parents[0])) {
        isLLC = true;
    }
}
...
uint64_t MESIBottomCC::processAccess(Address lineAddr, uint32_t lineId, AccessType type, uint64_t cycle, uint32_t srcId, uint32_t flags) {
    uint64_t respCycle = cycle;
    MESIState* state = &array[lineId];
    bool skipRecorder = flags & MemReq::PREFETCH_NOREC;
    ...
    switch (type) {
        ...
        case GETS:
            if (*state == I) {
                ...
                uint32_t netLat = getNetworkLatency(req);
                profGETNextLevelLat.inc(nextLevelLat);
                profGETNetLat.inc(netLat);
                respCycle += nextLevelLat + netLat;
                profGETSMiss.inc();
                if (!skipRecorder && !(flags & MemReq::PREFETCH)) {
...
void MESITopCC::init(const g_vector<BaseCache*>& _children, Network* _network, const char* _name) {
    ...
    if (!e->isEmpty()) {
        ...
        for (uint32_t c = 0; c < numChildren; c++) {
            if (e->sharers[c]) {
                ...
                respCycle += network ? network->getRTT(name.c_str(), children[c]->getName(), true) : 0;
