...
int main(int argc, char* argv[]) {
  ...
  Ramulator::Ramulator2Wrapper wrapper("/home/halfblood/common/temp/pouya_deps/ramulator2/tracer.yaml");

  int64_t addr = std::stoll(std::string(argv[1]));

  // sleep(5);
  // Ramulator::Ramulator2Wrapper wrapper2("/home/halfblood/common/temp/pouya_deps/ramulator2/example_config.yaml");
  int tick_count = 0;
  auto callback = [&tick_count](Ramulator::Request req) {
    std::cout << "request is done with no problemo at " << tick_count << std::endl;
    exit(0);
  };

  auto req = Ramulator::Request(addr, false, 0, callback);
  wrapper.enable_debug("test.trace");
  wrapper.send(req);
  while(true) {
    tick_count += 1;
    wrapper.tick();
