...
MemObject* BuildMemoryController(Config& config, uint32_t lineSize, uint32_t frequency, uint32_t domain, g_string& name) {
    ...
    if (type == "Simple") {
        ...
        // DAMOV-extended: core tracking option parsing
        ...
        mem = new Ramulator(ramulatorConfig, zinfo->numCores, lineSize, latency, domain, name, pimMode, application, frequency, record_memory_trace, networkOverhead, trackedCores);
